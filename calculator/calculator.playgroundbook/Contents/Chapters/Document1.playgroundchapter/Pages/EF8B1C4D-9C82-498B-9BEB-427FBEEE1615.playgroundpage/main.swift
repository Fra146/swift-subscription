...
//#-end-hidden-code
// Funzione da setUpLiveView.swift
setUpLiveView()

// Array contenenti le contrazioni
var αι = ["α+αι"]
var αυ = ["α+αυ"]
var ει = ["ε+ει", "ε+ε"]
...
let polbutlabel = Label(text: "Ricomincia Pol.", color: #colorLiteral(red: 0.0, green: 0.0, blue: 0.0, alpha: 1.0))
let polbutlabel2 = Label(text: "Genera Pol.", color: #colorLiteral(red: 0.0, green: 0.0, blue: 0.0, alpha: 1.0))
let polbutlabel3 = Label(text: "Calcola Pol.", color: #colorLiteral(red: 0.0, green: 0.0, blue: 0.0, alpha: 1.0))
let pollabel = Label(text: "Polinomio", color: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), font: .Futura, size: 30)
let label = Label(text: "Contrazioni", color: #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0), font: .Futura, size: 35)
...
// Piazzo i tasti minori e le labels
...
// Array per il calcolo delle contrazioni
var i = [String]()
// Array per il calcolo del polinomio
var p = [String]()

// Funzione addWord, chiamata ad ogni pressione di un tasto lettera o tasto +
...
// Funzione show. Mostra il risultato sulla label
...
// Funzione execute, chiamata da show al calcolo della contrazione. Controlla gli array
...
// Funzione createHandler, crea gli handler per i tasti delle lettere
func createHandler(button: Graphic, word: String) {
    button.setHandler(for: .touch) { _ in 
        ...
        addWord(word: word)
        if pollabel.text == "Ricominciato" {
            pollabel.text = "Polinomio"
        }
    }
...
// Handler tasto freccia 270 gradi
...
// Handler tasto Ricomincia Polinomio
polbut.setHandler(for: .touch) { _ in 
    ...
    p.removeAll()
    pollabel.text = "Ricominciato"
}
polbut.setHandler(for: .touchUp) { _ in
    polbut.backgroundColor = #colorLiteral(red: 0.250980406999588, green: 0.843137264251709, blue: 0.8235294818878174, alpha: 1.0)
}

// Handler tasto calcola polinomio
polbut3.setHandler(for: .touch) { _ in 
    ...
    if pollabel.text != "Polinomio" && pollabel.text != "Ricominciato" && Array(pollabel.text)[0] == "(" {
...
// Handler tasto genera polinomio
...
// Dichiarazione e piazzamento linee viola
